...
- **Typography.** Geist, self-hosted. The font file is committed at `apps/web/app/fonts/` under its SIL Open Font License, not fetched from a font service at build or at run time, so a build works with no network at all. Scale 12, 13, 14 (base), 16, 18, 24, 30. Tabular numerals in every grid and score display.
- **Spacing.** A 4 pixel grid. Comfortable and compact densities, per user.
- **Colour.** A neutral grey ramp plus one workspace-themeable brand hue, indigo by default. Semantic tokens for success, warning, danger and info. The status scale is deliberately saturated: on track, at risk and off track have to be legible at a glance, across a dense table, at chip size. Values, recipes and the full token table live in `docs/design/colour-system.md`; the five rules are here, because they are product behaviour rather than styling:
  1. **Red, amber and green are reserved for status.** Off track, at risk, on track. They never appear in the logo, in navigation, on a primary button, on a link, or as a decorative accent. If a green pixel is on screen, it means something is on track.
  2. **Progress is not health.** A progress bar fills with the brand hue, never a status colour. A key result can be at 90 percent and still be off track if the deadline is tomorrow; colouring the bar by health merges two independent variables and hides one. Progress is the bar, health is the chip beside it.
  3. **Not every number gets a colour.** Colour a metric only when the value itself carries good or bad meaning. Objectives at risk is a status. Check-in rate is an operational fact and stays neutral. Colouring everything stops the status colours working.
  4. **Never signal with colour alone.** Every status colour carries a text label or an icon. Health, confidence and corridor colours are token-driven and never the only signal.
  5. **Brand-coloured text uses the brand's text stop, not its fill stop.** A fill and a label have different contrast obligations and are different tokens.
- **Info is a blue of its own**, distinct from both the brand indigo and the status scale. It is the fourth semantic, not a fifth status: nothing informational may borrow green, amber or red.
- **Elevation.** A card is separated from the page by its border and its surface, not by a shadow. Shadows are for layers floating over arbitrary content: menus, dialogs, popovers.
...
**S-34 Onboarding.** After the first sign-in as owner: a five-step guided setup (name the workspace and its timezone, choose the check-in rhythm, invite somebody or skip, start from a template or empty, then explore with demo data or not). This line read four steps and put the template and the demo in one of them until 18 September 2026, when P8-T12 built the templates and they turned out to be two different offers: a template is a small first quarter somebody edits, the demo is a cast with months of history somebody looks at, and a single step cannot ask for both without a second control inside it. **Every step is skippable and every field arrives pre-filled with the TECHNICAL-PLAN.md §4.14 default**, so the owner can dismiss the whole flow and land in a working workspace. The flow confirms and refines defaults; it never gathers settings the product needs to function. A dismissed onboarding is resumable from admin, and no screen anywhere blocks on an unanswered setting. Per user on first visit: a five-stop tour covering the Work Map, the review inbox, a check-in, the cycle strip and ⌘K.
...
- The 3 to 1 floor binds the progress fill against its track and the focus indicator. It does not bind dividers, hairlines or an input's resting outline: those are boundaries between two surfaces, not graphics required to understand content. Muted text (placeholders, disabled labels) is the one text exemption WCAG allows, and it is never body copy.
- **Status dots are exempt, deliberately.** WCAG 1.4.11 asks 3 to 1 of a graphic "required to understand the content", and rule 4 guarantees a dot is never that: a status colour always ships with a text label, the label is what is announced, and the dot is `aria-hidden`. Holding the dots to 3 to 1 instead costs the scale its legibility — the amber that clears it on its own pill is an olive. The exemption is conditional on the structure that earns it, so `Chip` is tested to hide the dot from assistive technology and to carry a real label. If that stops being true, the ratio applies again and the tokens have to change.
- Every token pair the design system can put on screen is asserted on both themes by `packages/ui/test/tokens-contrast.test.ts`. A token edited to a value that drops a pair below its floor fails the build. That test is the contrast claim; a table in a document is not.
...
- [ ] Colour goes through a token, never a raw hex or `rgba`. No status colour on anything that is not a status: no health-coloured progress fill, no green success accent, no coloured metric that is not itself good or bad. Every status colour carries a label or an icon.
