...
> **Background only.** Nothing in the plan depends on this file, and it is never a spec source for a task. It is kept as a survey of what a full project-management suite contains, which is useful when deciding what OpenOKR deliberately leaves out.

What that system does, module by module. Companion to `legacy-data-model.md`.
