...
| `tasks`, `task_assignees`, `checklist_items` | Tasks, sub-tasks and task accesses | Status from the board column slug. Key result links preserved. Dependencies recorded in the report. `position` is renumbered on load rather than carried, because a source system's order is its own and the sparse spacing is this product's |
