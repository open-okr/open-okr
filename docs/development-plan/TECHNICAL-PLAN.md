...
| `blobs`, `attachments` | Task files | External file URLs become links in the body rather than copied bytes. **The bytes of a local file are not in the database at all**: `task_files` names a file on the FlowyTeam application server's own disk, and on the instance measured all 1535 rows are local files with no Google Drive, Dropbox or external address among them, so the path this row described is the one the live data never takes. Copying therefore needs a second input the command does not have, which is P6-T04c's first decision |
