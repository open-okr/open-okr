...
| R10 | Nothing consumes the outbox in a running instance | Every write enqueues its rows correctly and `OutboxRelay` exists in `packages/adapters`; what is missing is a deployment that constructs one. Owned by the deployment and operations phase, recorded here on 26 August 2026 rather than opened as a Phase 4 row, because no Phase 4 row can fix it. The visible consequences today: no content is indexed, so the copilot's retrieval finds nothing to cite even with a provider key, and no notification is delivered. P4-T13a's embedding worker and P2-T06's notification fan-out are both correct and both unreached, and P4-T14b-b is blocked on it | Run the relay inside the application process on a timer, which costs a single-instance deployment nothing and is wrong the moment there are two |
