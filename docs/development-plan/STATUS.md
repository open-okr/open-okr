...
| P2-T02 | can() + access-aware reads | todo |  |  | Carries a P1-T08 follow-up: workspace.overview resolves the reading member itself, because the access-aware getter does not exist yet. It is the same single seam the pipeline uses, so this task replaces both together. Carries a P1-T07 follow-up: the pipeline resolves the acting member and compares the action's declared access level, but every active member currently resolves to full because bindings do not exist. Replacing one function in operation.ts completes it; no handler changes |
