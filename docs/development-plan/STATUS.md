...
| P1-T06 | Workspaces + members bootstrap | in_progress | task/p1-t06-workspaces-members | 2026-08-06 | Human confirmed: enforce the registration policy now (open until a workspace exists, then invitation-only); no admin role invented, the first member's binding lands at P2-T01; active workspace is a signed cookie revalidated against memberships, no schema; provisioning is one explicit transaction shaped for the P1-T07 Operation lift, audit row deferred there |
| P1-T07 | Operation pipeline + action registry + audit spine | todo |  |  | Carries a P1-T05 follow-up: sign-in lockout is enforced but writes no audit entry, because the audit spine does not exist yet (TECHNICAL-PLAN §8.2 asks for both) |
...
| P2-T10 | App shell + design system | todo |  |  | Carries a P1-T05 follow-up: the authentication screens (S-35) ship with behaviour and semantics but no design system, and are restyled here along with the UIUX-PLAN §9 gates that need tokens and string catalogues |
