...
| P1-T04 | Adapter ports + drivers + the transactional outbox | in_progress | task/p1-t04-adapter-ports-outbox | 2026-08-05 | Human approved: ws as a runtime dependency for the realtime driver; drivers receive a pool by injection so the TECHNICAL-PLAN §1 dependency row stays true; the outbox table ships not-tenant-scoped and hard-delete markers per §4.13 |
