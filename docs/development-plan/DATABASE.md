...
|---|---|---|---|
| `outbox` | `topic`, `payload jsonb`, `idempotency_key` unique, `created_at`, `delivered_at?`, `attempts`, `available_at`, `last_error?`, `dead_lettered_at?`, `workspace_id?` | Not tenant scoped | The transactional side-effect queue. Only the relay reads it, and it must drain every workspace in one pass, which is why it has no row-level policy and gains none by carrying a workspace identifier. `workspace_id` (P8-T06b) is written by its own default, which reads the transaction's `app.workspace_id` setting, so nothing passes it and no write path can forget it; it is null on rows written outside a tenant-scoped transaction and on every row older than the column. **It orders the queue and authorises nothing**: the relay ranks each workspace's pending rows by age and takes rank one of every workspace before rank two, so one import's forty thousand rows no longer sit in front of another workspace's nudge. Tenant context a consumer needs still travels inside the payload. Delivered rows are purged by retention, so no soft delete. `dead_lettered_at` (P2-T06, closing a P1-hardening follow-up) is set once a row reaches `maxAttempts`: before it existed, a row that could never succeed retried on the lease interval forever, invisible to anyone, because nothing read `last_error` and `attempts` had no ceiling. `OutboxRelay.listDeadLettered()` is the visibility this was missing |
...
Unique on `token_hash` alone for live rows since migration 0075 (P6-G06b), and the sentence that used to stand here was the wrong assumption written down: "acceptance always names its workspace (by slug, in the URL) before it ever looks the token up, so the lookup is tenant-scoped like any other read rather than needing a global index". Nothing ever built that URL. `sendInvitation` has mailed a slug-less `<base>/join/<token>` since P1-T07, so which workspace a token belongs to is the first question acceptance has to answer rather than something it is told. The table therefore carries the **second-key policy** `api_tokens` has: a row is admitted through the ordinary tenant setting **or** through `app.invite_token_hash` matching its own digest, and the `with check` clause stays tenant-only so the pre-tenant key opens a read and never a write. The index is global because a digest has to name one row across the instance for that question to have an answer, which is also what makes two workspaces holding one digest a refusal rather than whichever row the planner reached first. `invitations.acceptLink` and `invitations.joinByTrustedDomain` (P2-T04) are both `bootstrap` operations: the person accepting has no member row yet, so there is nothing for the ordinary actor resolution to find, and the operation's own validation of the link (or of `workspaces.settings.trustedEmailDomains`) is the entire authorisation. Both funnel into the one shared `provisionMemberForInvite` in `packages/core/src/invitations/provisioning.ts`, which every joining path calls rather than inserting into `workspace_members` itself.
...
### outbox
`topic`, `payload jsonb`, `idempotency_key`, `created_at`, `delivered_at?`, `attempts`, `workspace_id?` (P8-T06b, for the relay's fair read order and nothing else).
...
### sso_connections *(P8-T07, migration 0091; SAML at P8-T07c-a, migration 0096)*
`workspace_id`, `kind`, `provider_id`, `display_name`, `discovery_url?`, `authorization_url?`, `token_url?`, `user_info_url?`, `client_id`, the three envelope-encryption columns for the client secret, `scopes`, `enforce`, `email_domains`, `enabled`, and for SAML `saml_entry_point?`, `saml_issuer?`, `saml_certificate?`, `saml_audience?`, `saml_want_assertions_signed`. Partial unique on `(workspace_id, provider_id) where deleted_at is null`, and `sso_connections_saml_complete` refuses a SAML row missing its entry point, issuer or certificate.

One row is one provider, OIDC or SAML. **This line said SAML arrives through a SAML-to-OIDC bridge, and that stopped being true at P8-T07c-a**, which mounted `@better-auth/sso` and added the columns above. `kind` defaults to `oidc`, so every row written before that migration keeps its meaning and no backfill runs. The certificate is deliberately not encrypted: it is a public key every identity provider publishes, and sealing it would imply that leaking it matters while the question that does matter, whether it is the right certificate, is answered by the tenant floor and by the admin screen that parses it before storing it.

**`sso_connections` is the authority and the plugin's `sso_providers` is derived from it.** Which identity provider a workspace trusts is business data, so it belongs on a table with `workspace_id` and a policy, and the plugin's own table has neither. `packages/core/src/auth/saml-sync.ts` is the one place that writes the derived row, from the authority rather than from its caller's arguments, and it is called at boot and again whenever a connection is created (P8-T07c-b, which found that nothing in the application called it at all). Nothing in the product reads `sso_providers` to answer a question, so a drifted row makes a sign-in fail, which somebody notices, rather than making an answer wrong, which nobody does.
...
`position` is `double precision` and sparse, spaced by 1024 (`TASK_POSITION_SPACING` in `packages/core`). A move reads its two new neighbours with three `limit 1` queries and writes the midpoint, so a drag touches one row. An advisory lock over the space's column serialises concurrent drags, rather than whichever board the drag happened on, because three boards read one set of rows. `ordering_state` records the spacing and the last renumber, so a reader can tell a fresh column from one dragged a hundred times without reading every row.

Both parts of that were measured at P7-T02 and both were wrong before it. The move used to row-lock every live card in the column: on §13.1's dataset a column holds 18,937 cards and a drag reached 20,394ms at the 95th percentile. Positions used to be whole numbers, so a gap of 1024 closed after ten drops into the same slot and the whole column was renumbered in that transaction, which left a fifteen-to-twenty-second tail on one drag in ten. Migration 0081 made the column fractional: the same gap now takes about forty halvings, and the renumber (`normaliseAndPlace`) stays for the case that reaches `MINIMUM_GAP`, one part in a billion. `double precision` rather than `numeric` because node-postgres returns a numeric as a string.
...
`subject_type?` and `subject_id?` since migration 0074 (P6-G07a), no foreign key for the same reason `activities.subject_id` has none. **The subject reached `notifyRecipients` and was thrown away.** It takes the subject to resolve who is watching and stored neither, so the only way back to what a row was about was through `activity_id`, which is nullable and null on two of the three producers: the Operation pipeline writes the activity *after* `execute` returns, so a fan-out inside `execute` has no id to record. `documents.publish` notifies the watchers of the document's parent rather than the document, and the task fan-out notifies a task's watchers, and neither can use the pipeline's own fan-out because that fans out on the activity's subject. So the inbox (S-03) could not group by entity or link to a target at all. Nullable, nothing backfilled, and the read coalesces the row's own subject with the activity's for rows written earlier.
...
Every column here is declared in the §4.14 registry since P6-G08, with `home: "notification_settings"`, and a test compares the registry's defaults against the column defaults for a member with no row. The registry's `scope` and its `home` are separate fields for exactly this table: these preferences are member-scoped and are not member columns, and `resolveMemberSettings` writes its answer into `workspace_members` at provisioning, so a key from here reaching it would name a column that does not exist. `routing` is a **partial** map: a reason absent falls back to the member's `primary_channel`, which is why its schema is `z.partialRecord` and not `z.record` — the latter is exhaustive over an enum key in Zod 4 and would demand all six reasons.
