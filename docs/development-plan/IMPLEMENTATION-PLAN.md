...
**P8-T06 was cut in four before any code, on 15 September 2026.** Its five
deliverables are four different kinds of work and only one of them had a
design: `p8-t01a-tenant-limits.md` covers the limits and says so in its own
§9, "a public status surface and per-tenant backup verification ... need
their own design, and it is not written here because this document is about
contention". Building all five in one session would have meant designing two
of them while implementing them, and reviewing a limiter, a public route, a
backup procedure and two documents in one diff.

The seam that matters most is between a and b. Everything in a is behind the
cloud flag and cannot change a self-hosted instance. Everything in b runs on
every deployment: the relay's read order and the connection pool's ceiling
are shipped code, and Agung's call on 15 September was that both should
change everywhere, because FIFO is unfair between workspaces on one server
too and `pg`'s default of ten connections is an accident rather than a
decision.

### P8-T06a: Per-tenant admission [M]
Depends on: P8-T05
Design: `docs/design/p8-t01a-tenant-limits.md` §3, §4, §6 and §7. Approved at the P8-T01a gate.
Deliverables: an admission step in `callAction` **before** the metrics span, so a refused call takes no pool connection; `cloud.limits.actionsPerMinute` and `cloud.limits.concurrentActions` in the TECHNICAL-PLAN §4.14 map at instance scope, both defaulting to 0 meaning unlimited; the concurrency counter taken on entry and released in a `finally`; the existing `rate_limited` refusal carrying the reset time on every surface, including the agent protocol and the chat commands, which no edge limiter reaches today.
Test plan: §8 criteria 2, 3, 5 and 6. Criterion 2 is the sharp one and is proved with the query-count budget from P7-T01a reporting zero statements for a refused call.
Acceptance: Given a tenant at its concurrency limit, when another of its members calls an action, then the refusal happens before a pool connection is taken; and given any limit at zero, when a tenant makes any number of calls, then none is refused.

### P8-T06b: The relay's round-robin read, and the pool ceiling [M]
Depends on: P8-T06a
Design: `docs/design/p8-t01a-tenant-limits.md` §5.
Deliverables: the outbox relay reading **round-robin by workspace** with a bounded slice per workspace per pass, rather than by insertion order; `cloud.limits.outboxRowsPerMinute` as the bound, at 0 meaning unlimited; a `max` on `apps/web/lib/pool.ts` as an environment setting with a documented default, since it has none today and runs on `pg`'s ten.
**This is the part that runs everywhere.** Neither change is behind the cloud flag: one workspace's import putting forty thousand jobs ahead of another workspace's nudge is the same unfairness on a self-hosted server, and a pool size nobody chose is not a self-host feature.
Test plan: §8 criteria 4 and 7. Criterion 7 is the one that keeps the fix honest: with only one workspace holding rows, throughput must not drop.
Acceptance: Given an import writing forty thousand outbox rows in one workspace, when a second workspace's nudge is enqueued behind it, then the nudge is delivered within its own window rather than after the import.

### P8-T06c: The public status surface and the capacity view [M]
Depends on: P8-T06b
**Design first.** Neither the status surface nor the capacity view is designed anywhere, and the limits document says so. Write `docs/design/p8-t06c-status-and-capacity.md` and get an explicit "design approved" before any code, the same rule as P3-T00, P4-T00, P5-T00 and P8-T01.
Deliverables, to be settled by that design: what a status surface says and to whom, given that it is public and must not leak which customers exist; whether it is served by the instance or by something outside it, because a status page hosted on the thing it reports on is a status page that goes down with it; and the capacity view, which is an extension of the observability profile P7-T06c already ships rather than a second dashboard system.
Acceptance: to be written in the design.

### P8-T06d: Per-tenant backup verification and the incident runbook [M]
Depends on: P8-T06c
**Design first**, for the same reason. `docs/runbooks/restore.md` covers restoring; nothing verifies that a backup would restore, which is the only claim a backup makes.
Deliverables, to be settled by that design: a per-tenant verification that actually loads a backup rather than checking a file exists; the incident runbook, which is the one deliverable of P8-T06 that is a document and nothing else.
Acceptance: to be written in the design.
...
### P8-T08a: SCIM through the Operation pipeline, deactivation, and groups [L]
Depends on: P8-T07a
...
Deliverables: `PATCH` and `DELETE` on the Users resource, deactivation mapping to suspension and never to deletion; every write through the Operation pipeline with its audit and activity rows; account creation through the one member funnel; the Groups resource mapped to space membership; `filter` on the list, because the identity providers use it to reconcile.

Acceptance: Given a user removed from the directory, when the next synchronisation runs, then the member is suspended and every token and grant of theirs stops working. Given a group with members, when it synchronises, then the space membership matches it.
