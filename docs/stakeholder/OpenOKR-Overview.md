...
| **Bring your own AI** | Anthropic, OpenAI, OpenRouter, Ollama or any OpenAI-compatible endpoint. Keys at deployment, workspace or per-user level, envelope-encrypted. A validated model catalogue with capability tiers, per-feature toggles, per-token cost metering, quotas, hard caps, versioned prompts, egress controls, and a zero-egress guarantee on local providers |
