...
describe("workspace provisioning wires the access model", () => {
  it("gives the workspace its own context, standard group, and the first member a full binding through their own group", async () => {
    ...
    // Four groups since P4-T05a: two `member` groups, the founding member's
    // and the seeded Champion's, plus the default space's `space_standard`
    // (P3-T01, whose membership is real data rather than structural) and the
    // workspace's own `workspace_standard`.
    expect(groups.rows).toHaveLength(4);
    ...
    // By member rather than by kind: there are two member groups now, and the
    // one this test is about belongs to the person who registered.
  ...
  it("does not duplicate the workspace_standard or member group on a second call", async () => {
    ...
    // Four, not two: the default space's own space_standard group is here
    // since P3-T01 and the Champion's member group since P4-T05a. Re-ensuring
    // the other two must not add a fifth.
    expect(groups.rows[0].n).toBe(4);
  });
