...
export const bindAgentScope = defineWriteAction({
  ...
  operation: (_context, input) => ({
    ...
    async execute({ tx, workspaceId, loaded }) {
      ...
      // privilege, P6-G13b, restored at P6-G13c). The summary above has said
      // "never workspace-wide" since P4-T05a and nothing enforced it:
      // `resolveSubjectContext` resolves `workspace` like any other subject,
      // so a caller passing that type got an agent with ambient authority
      // over everything.
      //
      // P6-G13b added this refusal and had to withdraw it the same day. The
      // floor in `runOperation` measured every actor against the workspace's
      // own context, so an agent bound only to a space held zero there and
      // could write nothing at all: enforcing the rule made `scoped_direct` a
      // mode no agent could act in. P6-G13c made a level on the subject clear
      // the floor as well, which is what lets this stand.
