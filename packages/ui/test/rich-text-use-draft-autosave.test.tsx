...
describe("useDraftAutosave", () => {
  test("recoveredDraft is null when nothing was ever saved", () => {
    const { result } = renderHook(() =>
      useDraftAutosave({ entityType: "comment", entityId: "e1", memberId: "m1", baseContent: BASE }),
    );
  ...
  test("save() writes to localStorage after the debounce, and recoveredDraft on a fresh mount reads it back", async () => {
    const { result } = renderHook(() =>
      useDraftAutosave({ entityType: "comment", entityId: "e1", memberId: "m1", baseContent: BASE, debounceMs: 10 }),
    );
    act(() => {
      result.current.save(EDITED);
    });
    await waitFor(() => {
      expect(window.localStorage.getItem(draftStorageKey("comment", "e1", "m1"))).not.toBeNull();
    });
  ...
  test("a draft against changed base content does not resurrect on mount", async () => {
    ...
    const changedBase = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "someone else's edit" }] }] };
    const { result: secondMount } = renderHook(() =>
      useDraftAutosave({ entityType: "comment", entityId: "e1", memberId: "m1", baseContent: changedBase }),
    );
    ...
    expect(window.localStorage.getItem(draftStorageKey("comment", "e1", "m1"))).toBeNull();
  });

  test("clearDraft removes it immediately, cancelling any pending debounced save", async () => {
    const { result } = renderHook(() =>
      useDraftAutosave({ entityType: "comment", entityId: "e1", memberId: "m1", baseContent: BASE, debounceMs: 50 }),
    );
    act(() => {
      result.current.save(EDITED);
