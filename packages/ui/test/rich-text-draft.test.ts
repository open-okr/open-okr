...
import { createDraft, draftStorageKey, fingerprint, isDraftUsable } from "../src/rich-text/draft.ts";
...
describe("fingerprint", () => {
  ...
  test("differs when content actually differs", () => {
    const a = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "a" }] }] };
    const b = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "b" }] }] };
...
describe("draftStorageKey", () => {
  test("is namespaced per entity and member", () => {
    expect(draftStorageKey("comment", "entity-1", "member-1")).toBe("openokr:draft:comment:entity-1:member-1");
  });
});

describe("isDraftUsable — a draft against changed base content does not resurrect", () => {
  const base = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "original" }] }] };
  const edited = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "in progress" }] }] };
  ...
  test("not usable once the base content has changed underneath it", () => {
    const draft = createDraft(edited, base, 1_000);
    const changedBase = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "someone else edited this" }] }] };
