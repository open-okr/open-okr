...
- `pnpm check:docs`: the documentation gate (P8-T11a). Every relative link under `docs/` resolves, every page in `docs/install`, `docs/admin` and `docs/runbooks` is reachable from `docs/README.md`, and every `pnpm <script>` a page tells somebody to run exists in `package.json`. The plan set, the design notes and the stakeholder pack are out of scope: they are working papers rather than pages a reader is sent to
