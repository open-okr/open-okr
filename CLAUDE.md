...
- **A red `packages/core` run may be the harness, not the code.** `workerDb()` opens with `drop database ... with (force)`, and Vitest reusing a pool slot number can make a new fork delete the database the outgoing fork is still reading. Every such failure is Postgres `57P01` and none of them is an assertion. Grep the output for `57P01` before believing a hundred failures, and re-run capped: `TEST_DB_PORT=5432 pnpm --filter @openokr/core exec vitest run --maxWorkers=4`. The full diagnosis and the fix somebody still has to make are in `docs/development-plan/PHASE-4-SPLIT.md`
